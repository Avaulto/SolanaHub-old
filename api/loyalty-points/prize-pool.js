export default async function getEstimatePrizePool(request, response) {
    const {showBreakdown} = request.query
    ...
    const weeklyRebates = async () => {
        ...
        const breakdown = { directStakeRebate: stakeRewardsWeeklyRebates, BLZEAirdrop: { weekly_BLZE_emmistion: BLZEAirdropWeeklyRebates.blzeAirdrop, BLZE_TO_SOL: BLZEAirdropWeeklyRebates.blze_to_sol_emmistions }};
        const stakeBoost = { totalRebates, APR_boost: await calcAPYboost(totalRebates)}
        showBreakdown ? stakeBoost.breakdown = breakdown : ''
        return stakeBoost
    }
